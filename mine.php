 ...
 if(!is_numeric($bet)) {
    ...
    if($_SESSION['hash']) {
      ...
      $result = array("info" => "false");
        ...
        $querym = ("UPDATE `users` SET `balance` = '$newbalance' WHERE `id` = '$id'");
...
if($row)
{
  $all_ref_money =$row['all_ref_money'];
$ref_money = $row['ref_money'];
}
$sumaref=$bet / 100 * 2;
 $update_sql12 = "Update users set all_ref_money=$sumaref + $all_ref_money WHERE id='$ref'";
    mysql_query($update_sql12);
 $update_sql12 = "Update users set ref_money=$sumaref + $ref_money WHERE id='$ref'";
    mysql_query($update_sql12);
}
...
// $stepi = 0;
// $arrm = ["d3a3fce9-33d5-4dc3-9956-576ccac153c5","363bd734-ae5c-40fa-8438-22bca75803f1","e9a77a18-b554-4311-afa9-81f0202d1e7f"];
//   $api = $arrm[0];
// $count = 0;
// while($count != $mines){
...
//             $rand = $link1['result']['random']['data'];
            
//             $random = $link1['result']['random'];
//             $random = json_encode($random);
//             $signature = $link1['result']['signature'];
...
mysql_query($querym);
        ...
        $query = ("INSERT INTO `mines-game` (`id_users`, `bet`, `onOff`, `step`,`result`, `win`,`mines`,`click`,`num_mines`,`login`,`random`,`signature`) VALUES ('$id', '$bet', '1', '0', '1', '0','$resultmines','$sss','$mines','$login','$random','$signature')");
        mysql_query($query);
...
$obj = array("info" => "warning","warning"=>"Ошибка, обратитесь в поддержку ");}}
    }else{
    ...
    $obj = array("info" => "warning","warning"=>"Кол-во бомб от 2 до 24");
  }
  }
...
$k = $win / $bet;
      ...
      mysql_query("Insert into kot_games ( `game`,`login`, `sum`, `chance`, `type`,  `win_summa`) 	VALUES ( 'MINES','{$login}', '{$bet}', '{$k}', 'win', '{$win}')");
  
          $update_sql2 = "Update users set wager =  $wager - ($win - $bet) WHERE hash='$sid'";
                mysql_query($update_sql2) or die("Ошибка вставки" . mysql_error());


      $query = ("UPDATE `users` SET `balance` = '$newbalance' WHERE `id` = '$id'"); //выдаем баланс игроку за победу.
      mysql_query($query);
  
    $q = ("SELECT * FROM `mines-game` WHERE `id_users` = '$id' AND `onOff` = '1' ");
      $mi = mysql_query($q);
  ...
  // для работы с антиминусом
   ...
   $query = mysql_query("UPDATE `admin_mines` SET `bank`= '$bank'-'$win'");
    ...
    $obj = array("info"=>"true","caef"=>"$caef","game_id"=>"$game_id","win" => "$win","money"=>"$money","tamines"=>"$tamines","resultHistoryContentBomb"=>"$ssa");
  }else{
...
$query = ("SELECT * FROM `mines-game` WHERE `id_users` = '$id' AND `onOff` = '1'");
   ...
   $game_id = $adm['id'];
    ...
    if(in_array($mmines,$mines)){
     ...
     $click[] = $mmines;
      ...
      mysql_query("Insert into kot_games ( `game`,`login`, `sum`, `chance`,`type`, `win_summa`) 	VALUES ( 'MINES','{$login}', '{$bet}', 'lose', '0', '0')");
   ...
   $query = mysql_query("UPDATE `admin_mines` SET `bank`= '$bank'+'$bank1',`zarabotok`='$profit'+'$profit1'");
      ...
      $obj = array("info" => "click","game_id" =>"$game_id","bombs"=>"true","pressmine" => "$mmines","tamines"=>"$tamines","resultHistoryContentBomb"=>"$saad");
...
if($num_mines == 4){
      ...
      $obj = array("info" => "click","bombs"=>"false","step"=>$step+1,"pressmine" => "$mmines","win" => "$win","gem"=>"$gem_number","nextX"=>"$nextCoef","resultHistoryContentBomb"=>"$rdr");
